"""Layer 2 — deterministic AST validation with sqlglot. No LLM involved.

Rule of thumb: **reject unless proven safe.** The generated SQL is accepted only
when it parses to exactly one read-only ``SELECT`` (or ``WITH ... SELECT``) whose
entire tree is free of DML/DDL/command nodes, dangerous functions, system-catalog
access, and write-producing constructs (``SELECT INTO``, ``... RETURNING`` inside
a data-modifying CTE, etc.).
"""
