"""Layer 3 (optional, pre-LLM) — NL-input injection & intent defense.

When you're generating SQL from a natural-language question, the *question* is
untrusted input. Screen it before it reaches the model: prompt-injection ("ignore
your instructions..."), destructive intent (delete/drop/update...), system-catalog
probing, dangerous functions, credential/exfiltration requests, tenant-escape,
and resource-abuse / unbounded-scan intent. Allow-list awareness lets it flag
probes for tables/columns that don't exist without blocking legitimate questions.

Deterministic and explainable — every block cites the rule that fired. Pair it
with a constrained LLM intent-classifier if you want; this is the fast, auditable
first pass.
"""
...
def screen_question(question: str, allow_list: AllowList | None = None) -> ScreenResult:
    """Screen an untrusted NL question for unsafe intent before SQL generation."""
    reasons: list[str] = []
    rules: list[str] = []
    controls: list[str] = []
    ...
    if allow_list is not None:
        ...
        for m in _SNAKE.finditer(question):
            ...
            reasons.append(f"'{tok}' is not a known table or column")
